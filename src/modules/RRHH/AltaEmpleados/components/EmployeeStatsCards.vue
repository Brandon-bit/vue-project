...
const vacationPercentage = computed(() => {
    if (stats.value.total === 0) return '0.0'
    return ((stats.value.onVacation / stats.value.total) * 100).toFixed(1)
})
...
<template>
    <div class="grid gap-6 md:grid-cols-4 mb-8">
        ...
        <!-- De Vacaciones -->
        <div class="stat-card group">
            <div class="stat-icon-wrapper bg-warning/10">
                <span class="material-symbols-outlined text-warning text-2xl">beach_access</span>
            </div>
            <div class="stat-content">
                <h3 class="stat-label">De Vacaciones</h3>
                <div class="stat-value text-warning">{{ stats.onVacation }}</div>
                <p class="stat-description">{{ vacationPercentage }}% de la plantilla</p>
            </div>
            <div class="stat-decoration bg-warning/5"></div>
        </div>
