import { z } from 'zod'

const entryItemSchema = z.object({
    id: z.string(),
    account: z.string().min(1, 'La cuenta es requerida'),
    description: z.string().min(1, 'La descripción es requerida'),
    debit: z.number().min(0, 'El debe debe ser mayor o igual a 0'),
    credit: z.number().min(0, 'El haber debe ser mayor o igual a 0'),
    reference: z.string().optional()
})

export const expensePolicySchema = z.object({
    date: z
        .string({ required_error: 'La fecha es requerida' })
        .min(1, 'La fecha es requerida'),
    supplier: z
        .string({ required_error: 'El proveedor es requerido' })
        .min(1, 'Debe seleccionar un proveedor'),
    concept: z
        .string({ required_error: 'El concepto es requerido' })
        .min(10, 'El concepto debe tener al menos 10 caracteres'),
    expenseType: z
        .string({ required_error: 'El tipo de gasto es requerido' })
        .min(1, 'Debe seleccionar un tipo de gasto'),
    costCenter: z
        .string({ required_error: 'El centro de costo es requerido' })
        .min(1, 'Debe seleccionar un centro de costo'),
    paymentMethod: z
        .string({ required_error: 'El método de pago es requerido' })
        .min(1, 'Debe seleccionar un método de pago'),
    bankAccount: z
        .string({ required_error: 'La cuenta bancaria es requerida' })
        .min(1, 'Debe seleccionar una cuenta bancaria'),
    paymentReference: z
        .string()
        .optional(),
    entries: z
        .array(entryItemSchema)
        .min(2, 'Debe agregar al menos 2 partidas')
        .refine(
            (entries) => {
                const totalDebit = entries.reduce((sum, e) => sum + e.debit, 0)
                const totalCredit = entries.reduce((sum, e) => sum + e.credit, 0)
                return Math.abs(totalDebit - totalCredit) < 0.01
            },
            {
                message: 'La póliza debe estar cuadrada (Debe = Haber)'
            }
        )
})
